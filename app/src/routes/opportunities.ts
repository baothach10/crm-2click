...
import {
  getHandoffRun,
  getHandoffRunSteps,
  listHandoffRunsForOpportunity,
  type HandoffRunStepRow,
  type HandoffRunSummary,
} from "../db/queries/handoff.js";
import {
...
import { ENGINE_LABEL } from "../handoff/deterministicStandIn.js";
import { OpportunityNotFoundError, runHandoffAssistant } from "../handoff/coordinator.js";
import type { Brief, Finding, Review } from "../handoff/types.js";
...
import { decisionBadge, emptyState, page, statusBadge } from "../web/shell.js";
...
function handoffRunRow(opportunityCode: string, run: HandoffRunSummary): SafeHtml {
  return html`<tr>
    <td>${formatDateTime(run.created_at)}</td>
    <td>${decisionBadge(run.decision)}</td>
    <td>${run.iterations}</td>
    <td>${run.reason}</td>
    <td><a href="/opportunities/${opportunityCode}/handoff/${run.id}">Details →</a></td>
  </tr>`;
}

function findingList(findings: Finding[]): SafeHtml {
  return html`<ul>${join(
    findings.map(
      (f) =>
        html`<li>${f.severity === "blocker" ? html`<span class="badge badge-danger">blocker</span> ` : raw("")}${f.message}</li>`,
    ),
  )}</ul>`;
}

function handoffStepCard(step: HandoffRunStepRow): SafeHtml {
  if (step.role === "preparer") {
    const brief = step.output as Brief;
    return html`<div class="panel">
      <p><strong>Preparer</strong> (iteration ${step.iteration}) → ${decisionBadge(brief.decision)}</p>
      ${brief.missing_fields.length > 0
        ? html`<p class="muted">Missing fields: ${brief.missing_fields.join(", ")}</p>`
        : raw("")}
      <p>${brief.proposed_next_step}</p>
      ${brief.note ? html`<p class="notice notice-info">${brief.note}</p>` : raw("")}
    </div>`;
  }
  const review = step.output as Review;
  return html`<div class="panel">
    <p><strong>Checker</strong> (iteration ${step.iteration}) → ${decisionBadge(review.decision)}
      <span class="badge ${review.verdict === "approved" ? "badge-ok" : "badge-warn"}">${review.verdict}</span></p>
...
export function registerOpportunityRoutes(app: FastifyInstance): void {
  app.get<{ Params: { code: string }; Querystring: { activity_before_at?: string; activity_before_id?: string } }>(
    ...
    async (req, reply) => {
      ...
      const [activity, followUps, otherEditions, contacts, reps, handoffRuns] = await Promise.all([
        ...
        listSalesReps(),
        listHandoffRunsForOpportunity(opp.id),
      ]);
      ...
      const body = html`
        ...
        <h2>Handoff assistant</h2>
        <p class="muted">
          Prepares a brief for the technical team, checks it against the fair information, and decides
          whether to hand off, give early notice only, or hold for missing or conflicting information.
          Generated by a deterministic local stand-in (${ENGINE_LABEL}) — no language model is called.
        </p>
        <form method="post" action="/opportunities/${opp.opportunity_code}/handoff">
          <button type="submit">${handoffRuns.length === 0 ? "Run assistant" : "Run assistant again"}</button>
        </form>
        ${handoffRuns.length === 0
          ? emptyState("No runs yet.")
          : html`<div class="panel table-scroll">
              <table>
                <thead><tr><th>When</th><th>Decision</th><th>Iterations</th><th>Reason</th><th></th></tr></thead>
                <tbody>${join(handoffRuns.map((r) => handoffRunRow(opp.opportunity_code, r)))}</tbody>
              </table>
            </div>`}
  ...
  app.post<{ Params: { code: string } }>("/opportunities/:code/handoff", async (req, reply) => {
    const opp = await getOpportunityByCode(req.params.code);
    if (!opp) {
      reply.code(404).send("Opportunity not found");
      return;
    ...
    try {
      const runId = await runHandoffAssistant(opp.id);
      reply.redirect(`/opportunities/${req.params.code}/handoff/${runId}`);
    } catch (err) {
  ...
  app.get<{ Params: { code: string; runId: string } }>(
    "/opportunities/:code/handoff/:runId",
    async (req, reply) => {
      const run = await getHandoffRun(Number(req.params.runId));
      if (!run || run.opportunity_code !== req.params.code) {
        reply.code(404).type("text/html").send(page("Not found", html`<h1>Run not found</h1>`));
        return;
      }
      const steps = await getHandoffRunSteps(run.id);

      const body = html`
        <p class="muted"><a href="/opportunities/${run.opportunity_code}">← Back to ${run.opportunity_code}</a></p>
        <h1>Handoff assistant — run #${run.id}</h1>
        <p>${decisionBadge(run.decision)} <span class="muted">${formatDateTime(run.created_at)} · ${run.iterations} iteration${run.iterations === 1 ? "" : "s"} · ${run.engine} · policy ${run.policy_version}</span></p>
        <div class="panel"><strong>Reason:</strong> ${run.reason}</div>

        <h2>Role outputs</h2>
        ${join(steps.map(handoffStepCard))}

        <h2>Information used</h2>
        <p class="muted">Frozen at the time of this run; the opportunity may have changed since.</p>
        <div class="panel"><pre style="white-space:pre-wrap;word-break:break-word;margin:0">${JSON.stringify(run.input_snapshot, null, 2)}</pre></div>
      `;

      reply.type("text/html").send(page(`Handoff run #${run.id}`, body));
    },
  );
}
